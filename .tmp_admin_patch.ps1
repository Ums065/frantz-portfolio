$path = Resolve-Path 'frontend/src/pages/Admin.tsx'
$text = [System.IO.File]::ReadAllText($path)

$pattern = '(?s)  const filteredSchools = useMemo\(\(\) => nsSchools\.filter\(\(school: any\) => \{.*?\}\), \[nsSchools, schoolDashFilter\]\)'
$replacement = @"
  const filteredSchools = useMemo(() => nsSchools.filter((school: any) => {
    if (schoolDashFilter === 'all') return true
    if (schoolDashFilter === 'trendcatch') return String(school.origin || '') === 'trendcatch_edu'
    if (schoolDashFilter === 'claimed') return String(school.claim_status || '') === 'claimed'
    if (schoolDashFilter === 'unclaimed') return String(school.claim_status || '') !== 'claimed'
    return String(school.status || '') === schoolDashFilter
  }), [nsSchools, schoolDashFilter])
  const schoolMap = useMemo(() => new Map<number, any>(nsSchools.map((school: any) => [Number(school.id), school])), [nsSchools])
"@
$text = [regex]::Replace($text, $pattern, $replacement, 1)

$pattern = '(?s)            <DataTable\r?\n              head=\{\[''#'', ''Name'', ''User ID'', ''Email'', ''Role'', ''Status'', ''Reviewed'', ''''\]\}.*?            />'
$replacement = @"
            <DataTable
...
$text = $text.Replace("                    <p>{(s.school_district || '—')} · {s.status}</p>", "                    <p>{[s.school_district || '—', s.origin === 'trendcatch_edu' ? 'TrendCatch EDU' : 'Principal', s.claim_status === 'claimed' ? 'Claimed' : s.status].filter(Boolean).join(' · ')}</p>")
$text = $text.Replace("                      {selectedDashSchool.school_address && <span><b>Address:</b> {selectedDashSchool.school_address}</span>}", "                      {selectedDashSchool.school_address && <span><b>Address:</b> {selectedDashSchool.school_address}</span>`r`n                      {selectedDashSchool.school_website && <span><b>Website:</b> {selectedDashSchool.school_website}</span>`r`n                      <span><b>Origin:</b> {selectedDashSchool.origin === 'trendcatch_edu' ? 'TrendCatch EDU' : 'Principal'}</span>`r`n                      <span><b>Claim:</b> {selectedDashSchool.claim_status === 'claimed' ? 'Claimed' : 'Unclaimed'}</span>")

$pattern = '(?s)            <p className="ns-edu-intro">.*?</p>\r?\n'
$replacement = @"
...
$text = $text.Replace('                      <p className="ns-edu-card__meta">{school.administrator_email || ''no email''}{school.school_website ? ` · ${school.school_website}` : ''''}</p>', '                      <p className="ns-edu-card__meta">{school.administrator_email || ''no email''}{school.school_website ? ` · ${school.school_website}` : ''''}</p>`r`n                      <p className="ns-edu-card__meta">{[school.school_district, school.main_phone, school.principal_name].filter(Boolean).join('' · '') || ''Awaiting school details''}</p>')
$text = $text.Replace('                      <button type="button" className="btn btn--sm" onClick={() => setClaimSchoolId(claiming ? null : school.id)}>', '                      {school.status !== ''rejected'' && school.status !== ''approved'' && (`r`n                        <button type="button" className="btn btn--sm" disabled={nsEduBusy === `reject-${school.id}`} onClick={() => void eduRejectSchool(school.id)}>`r`n                          {nsEduBusy === `reject-${school.id}` ? ''Saving...'' : ''Reject''}`r`n                        </button>`r`n                      )}`r`n                      <button type="button" className="btn btn--sm" onClick={() => setClaimSchoolId(claiming ? null : school.id)}>')
$text = $text.Replace('<label>Principal name<input name="principal_name" required /></label>', '<label>Principal name<input name="principal_name" required defaultValue={school.principal_name || ''} /></label>')
$text = $text.Replace('<label>Administrator name<input name="administrator_name" placeholder="(defaults to principal)" /></label>', '<label>Administrator name<input name="administrator_name" placeholder="(defaults to principal)" defaultValue={school.administrator_name || ''} /></label>')
$text = $text.Replace('<label>Principal phone<input name="administrator_phone" required /></label>', '<label>Principal phone<input name="administrator_phone" required defaultValue={school.main_phone || ''} /></label>')
$text = $text.Replace('<label>Main phone<input name="main_phone" placeholder="(defaults to principal phone)" /></label>', '<label>Main phone<input name="main_phone" placeholder="(defaults to principal phone)" defaultValue={school.main_phone || ''} /></label>')
$text = $text.Replace('<label>District<input name="school_district" required /></label>', '<label>District<input name="school_district" required defaultValue={school.school_district || ''} /></label>')
$text = $text.Replace('<label>Address<input name="school_address" required /></label>', '<label>Address<input name="school_address" required defaultValue={school.school_address || ''} /></label>')
