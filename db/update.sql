...
-- requests, and role-targeted announcements. Mirrors ecosystem_ensure_schema() and
-- ecosystem_shared_ensure_schema() in api/lib.php. No FK to users (matches the app).
...
-- (implementation help / contact school / hiring / volunteer). Businesses cannot
-- score students. Mirrors business_ensure_schema() in api/lib.php.
...
-- Business <-> interview matching link columns (explicit admin link + recorded URL).
CALL add_column_if_missing('new_school_business_interviews', 'business_user_id', 'INT DEFAULT NULL', 'is_starred');
...
-- Distinct from the ecosystem Partner login dashboard. Mirrors
