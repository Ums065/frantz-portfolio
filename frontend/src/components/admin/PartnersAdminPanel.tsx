          ...
          <div className="modal" style={{ maxWidth: 640, width: '96vw', maxHeight: '92vh', overflowY: 'auto', textAlign: 'left', padding: 0, background: 'var(--bg-2, #14130d)' }}>
            ...
            <div style={{ padding: '18px 22px', display: 'grid', gap: 12 }}>
              <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                ...
                <label className="btn btn--sm" style={{ cursor: 'pointer' }}>{busy ? 'Uploading…' : 'Upload logo'}<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" hidden onChange={(e) => { const f = e.target.files?.[0] || null; e.target.value = ''; uploadLogo(f) }} /></label>
              </div>
